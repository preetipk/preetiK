...
app.get('/user', function(req, res) {
    ...
    async.series([
        function(callback) {
            User.find({ $or: [{ status: 'activated' }, { status: "deactivated" }], $comment: "Find even values." }, function(err, docs) {
                //console.log(docs);
                callback(null, docs);
            })
        }
...
User.count({ status: 'activated' }, function(err, count) {
...
app.delete('/user/:id', function(req, res) {
    ...
    User.findByIdAndRemove({ "_id": id }, function(err, docs) {
        ...
        if (docs.length !== 0) {
            User.remove({ "_id": id }, function(err) {
                if (err) {
                    ...
                    res.send('DataDeleted Successfully')
                }
            })
        } else {
